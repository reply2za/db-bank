...
exports.run = async (event: MessageEventLocal) => {
    if (event.args.length < 1) {
        ...
        if (bidAmt <= 0) {
            event.message.channel.send("you do not have a max bid set");
        } else {
            event.message.channel.send("you set today's max bid to: $" + bidAmt);
        }
    } else {
        ...
        if (bidAmt && !isNaN(bidAmt) && bidAmt > 0.25) {
            ...
            if (!bankUser) { 
                event.message.channel.send("there was an error setting the max bid");
                return;
            }
            event.message.channel.send("today's max bid of $" + bidAmt + " has been set!");
            const data = new Map();
            ...
            event.message.channel.send("bid amount must be a positive number greater than .25");
        }
    }
}