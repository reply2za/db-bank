...
exports.run = async (event: MessageEventLocal): Promise<void> => {
    ...
    if (!msg) {
        await (<TextChannel>event.channel).send('*could not find author*');
        return;
    }
    const numToDelete = parseInt(event.args[1]);
    if (!numToDelete || numToDelete > NUM_TO_FETCH) {
        await (<TextChannel>event.channel).send('*invalid number*');
        return;
    }
    const numRemoved = await removeDBMessage(msg.channel.id, 1, true);
    (<TextChannel>event.channel).send(`deleted ${numRemoved} additional messages`);
};
